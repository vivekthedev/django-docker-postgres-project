...
class BookTests(TestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(
            username="reviewuser", email="review@email.com", password="testpass123"
        )
        ...
        self.review = Review.objects.create(
            book=self.book, author=self.user, review="Shitty book"
        )
    ...
    def test_book_detail_view_with_persmissions(self):
        self.client.login(email='review@email.com',password='testpass123')
        ...
        self.assertContains(response, "Harry Potter")
        self.assertContains(response, "Shitty book")
